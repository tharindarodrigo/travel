        ...
        if ($roomfacility = RoomFacility::create($data)) {
            if(Input::file('icon')){
                ...
                Image::make(Input::file('icon'))
                    ...
                    })->save('control-panel-assets/images/room-facilities/'.$roomfacility->id.'.png');
            }
...
//                dd('asdasd');
                File::delete('control-panel-assets/images/room-facilities/'.$id.'.png');

                Image::make(Input::file('icon'))
                    ->encode('png')
                    ->resize(32, 32, function ($constraint) {
                        $constraint->aspectRatio();
                    })->save('control-panel-assets/images/room-facilities/'.$id.'.png');
            }
