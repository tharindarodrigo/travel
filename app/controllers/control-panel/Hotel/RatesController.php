...
class RatesController extends \BaseController
{
    ...
    public function insertRates($newperiods, $data)
    {
        foreach ($newperiods as $newperiod) {
            ...
            $duplicate_rate = Rate::where($data_wo_user_id)->get();
    ...
    public function edit($hotelid, $id)
    {
        ...
        foreach ($rates as $marketid) {
            if (!in_array($marketid->id, $checkedmarketids)) {
                $checkedmarketids[] = $marketid->market_id;
            }
        }


        return View::make('control-panel.hotel.rates.edit', compact('hotelid', 'rate', 'rates', 'mealbases', 'checkedmarketids', 'roomtype'));
    }
    ...
    public function updateRates($hotelid)
    {
